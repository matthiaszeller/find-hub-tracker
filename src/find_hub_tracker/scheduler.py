...
class Scheduler:
    ...
    def run_once(self, fun: Callable, *, id: str, wait_seconds: float = 0.0) -> None:
        ...
        *,
        ...
        self._add_job(
            ...
            trigger=IntervalTrigger(
                seconds=interval.total_seconds(), start_time=start_time
            ),
            id=id,
        )
        ...
        try:
            ...
            try:
                await asyncio.wait_for(
                    aps.__aexit__(None, None, None), timeout=self.shutdown_grace_seconds
                )
            except TimeoutError:
                log.error(
                    "shutdown_grace_period_exceeded",
                    message="A job didn't stop in time, forcing exit",
                )
                os._exit(1)
